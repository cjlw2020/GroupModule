...
las1 <- readLAS("/var/folders/7g/dn_1ng7x3rvgnnf8c97vn_6h0000gp/T//Rtmpl2Ufea/file54ee4d3fa607.las") #you might need to change this to the path given in the output above under 'saved locally as:'
las2 <- readLAS("/var/folders/7g/dn_1ng7x3rvgnnf8c97vn_6h0000gp/T//Rtmpl2Ufea/file54eedad3d30.las")
...
chm1 <- grid_canopy(las, res = 1, dsmtin())
...
```{r eval=FALSE}
slope  <- terrain(chm1, opt='slope')
aspect <- terrain(chm1, opt='aspect')
hill <- hillShade(slope, aspect, 45, 120)
plot(hill, col=grey(0:100/100), legend=FALSE, main='Boston University')
plot(chm1, col=rainbow(25, alpha=0.35),add=TRUE) # 2 min
```
...
chm2 <- projectRaster(chm1, crs = crs(BU_buildings_shp))
chm3 <- crop(chm2, BU_buildings_shp) 

hill2 <- projectRaster(hill, crs = crs(BU_buildings_shp))
hill3 <- crop(hill2, BU_buildings_shp) 

plot(hill3, col=grey(0:100/100), legend=FALSE, main ='Boston University')
plot(BU_buildings_shp,add=TRUE)
plot(chm3,col=rainbow(25, alpha=0.35),add=TRUE) # 30 sec
```
...
las.bc1 <- readLAS("/var/folders/7g/dn_1ng7x3rvgnnf8c97vn_6h0000gp/T//Rtmpl2Ufea/file54ee74b0f765.las") #you might need to change this to the path given in the output above under 'saved locally as:'
las.bc2 <- readLAS("/var/folders/7g/dn_1ng7x3rvgnnf8c97vn_6h0000gp/T//RtmpxdwhDl/file51f4184f095f.las")
...
proj4string(las.bc) <- CRS("+proj=utm +zone=19 +datum=NAD83") #these specifications are in las download metadata text file
...
las.aa <- readLAS("/var/folders/7g/dn_1ng7x3rvgnnf8c97vn_6h0000gp/T//RtmpxdwhDl/file51f46bf81953.las") #you might need to change this to the path given in the output above under 'saved locally as:'
